...
public class CarRange {
	...
	void read(int a, int b, int c, int d, int e, int f, int g, int h, int list[]) {
		...
		for(int i : list) {
			if(i == 0) fuelList.add("가솔린");
			if(i == 2) fuelList.add("LPG");
			if(i == 4) fuelList.add("디젤");
			if(i == 6) fuelList.add("전기");
			if(i == 8) fuelList.add("하이브리드");
		}
	}
	
	void print() {
		System.out.printf("%d %d %d %d %d %d %d %d",this.startPrice,this.endPrice,this.startYear,this.endYear,this.startFuelEffi,this.endFuelEffi
				,this.startPower,this.endPower);
	}
}
